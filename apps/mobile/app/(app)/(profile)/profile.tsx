...
import DecoSVG from "../../../assets/svg/deco.svg";
                    ...
                    <VStack className="items-center mt-4 h-full flex w-full" space="xl">
                        {user ? (
                          <View className="relative">
                              ...
                              >
                                  <Avatar size="2xl" className="p-[2px] bg-rythmix-black">
                                      <AvatarFallbackText>
                        ...
                        <VStack className="items-center" space="sm">
                            <Heading size={"4xl"} className="font-black text-rythmix-white">
                                {user?.username}
                            </Heading>
                            <Text size="md" className="text-rythmix-white">{user?.email}</Text>
                            <HStack className={"items-center gap-2"}>
                                <Text size="md" className="text-rythmix-white">Lvl. 4</Text>
                        ...
                        <VStack className={"w-full gap-4 mt-4 px-6"}>
                            ...
                            { user?.description && (
                              <HStack className={"items-center px-4 py-6 border-rythmix-white border-[6px] bg-[#54555C]"}>
                                  <Text className="text-rythmix-white font-regular mr-2">{user?.description}</Text>
                              </HStack>
                            )}
                            <HStack className={"items-center px-4 py-6 border-rythmix-white border-[6px] bg-[#54555C]"}>
                                <Text className="text-rythmix-white font-regular mr-2">Member since {formatDate(user?.createdAt!)}</Text>
                            </HStack>
                            <Heading size={"2xl"} className={"text-rythmix-white font-black"}>Profile management</Heading>
                            <View style={{ boxShadow: "0px 4px 4px 0px #00000025" }}>
                                ...
                                >
                                    <TouchableOpacity className="relative bg-rythmix-white p-4 justify-center m-1 h-16" onPress={handleEditClick}>
                                        <HStack className={"items-center justify-between"}>
                                            <GradientText block={true} variant="secondary" className="uppercase font-black" style={{ fontSize : 24}}>Edit my profile</GradientText>
                                        </HStack>
                            ...
                            <VStack style={{ boxShadow: "0px 4px 4px 0px #00000025" }} className={"border border-rythmix-primary bg-rythmix-primary"}>
                                <TouchableOpacity className={"bg-rythmix-primary p-4 justify-center m-1 h-16"} onPress={() => logout()}>
                                    <HStack className={"items-center justify-between"}>
                                        <Text className="uppercase text-rythmix-white font-black text-2xl">Sign out</Text>
                                        <DecoSVG />
                                    </HStack>
                                </TouchableOpacity>
                            </VStack>
